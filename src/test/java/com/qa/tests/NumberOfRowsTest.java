...
public class NumberOfRowsTest extends TestBase {
	...
	public void numberOfRows(Hashtable<String, String> data) {
		extentTest = extent.startTest("verifyNumberOfRowsTest");
		if (data.get("Runmode").equals("N")) {
			throw new SkipException("Skipping the test as Runmode was NO");
		}
		loginPage.tapLogin(data.get("email"), data.get("password"));
		homePage.tapOnTradeLink();
		homePage.scrollElement("Crypto");
		Assert.assertEquals(loginPage.verifyRows(), 20);
	}

	@Test(priority = 2, dataProvider = "getData")
	public void randomRows(Hashtable<String, String> data) {
		extentTest = extent.startTest("verifyNumberOfRowsTest");
		if (data.get("Runmode").equals("N")) {
			throw new SkipException("Skipping the test as Runmode was NO");
		}
		...
		homePage.tapOnTradeLink();
		homePage.scrollElement("Crypto");
		homePage.selectRandomRow();
		homePage.scrollElement("Favorites");
		homePage.verifyData();
	}
	...
	public Object[][] getData(Method m) {
		switch (m.getName()) {
		case "numberOfRows":
			return DataUtil.getData(numberOfRows, xls);
		case "randomRows":
			return DataUtil.getData(numberOfRows, xls);
		}
		return null;
